...
import com.zenith.util.config.Config.Client.Extra.PearlPlus;
...
public class AutoDetectModule extends Module {
    ...
    public boolean enabledSetting() {
        return CONFIG.client.extra.pearlPlus.autoDetect.enabled;
    }
    ...
    public List<EventConsumer<?>> registerEvents() {
        return List.of(
                of(ClientBotTick.class, event -> {
                    if (!CONFIG.client.extra.pearlPlus.autoDetect.enabled) {
                        return;
                    }
                    scanForPearls();
                }),
    ...
    public boolean isTemporaryModeEnabled() {
        return CONFIG.client.extra.pearlPlus.autoDetect.temporaryMode;
    }
    ...
    private Optional<OwnerInfo> resolveOwnerInfo(Entity pearl, Map<Integer, Entity> entities) {
        ...
        if (resolved.isPresent() || !CONFIG.client.extra.pearlPlus.autoDetect.distanceCheck) {
            return resolved;
        }
    ...
    private void checkStoredPearlsForMissingEntities(Map<Integer, Entity> entities) {
        ...
        if (CONFIG.client.extra.pearlPlus.players.isEmpty()) {
            return;
        }
        Proxy proxy = Proxy.getInstance();
        if (proxy == null || !proxy.isConnected() || proxy.isInQueue()) {
        ...
        for (var entry : new HashMap<>(CONFIG.client.extra.pearlPlus.players).entrySet()) {
            ...
            for (var pearlEntry : new HashMap<>(playerPearls.pearls).entrySet()) {
                PearlPlus.StoredPearl stored = pearlEntry.getValue();
                if (stored == null) {
                    continue;
                }
    ...
    private Optional<StoredPearlEntry> findStoredPearlByColumn(int blockX, int blockZ) {
        return CONFIG.client.extra.pearlPlus.players.entrySet().stream()
    ...
    private record StoredPearlEntry(UUID ownerUuid, String ownerName, PearlPlus.StoredPearl pearl) {
        OwnerInfo ownerInfo() {
