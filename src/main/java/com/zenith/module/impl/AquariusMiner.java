...
import com.zenith.util.config.Config.Client.Extra.AquariusMiner.AreaAnchor;
import com.zenith.util.config.Config.Client.Extra.AquariusMiner.AreaMode;
...
public class AquariusMiner extends Module {
     ...
     * An ender chest only drops itself when mined with SILK TOUCH; ZenithProxy's auto-tool picks the FASTEST
    ...
    private boolean isSilkPick(@Nullable ItemStack s) {
        ...
        return ench != null && ench.getEnchantments().containsKey(com.zenith.mc.enchantment.EnchantmentRegistry.SILK_TOUCH.get().id());
    }
