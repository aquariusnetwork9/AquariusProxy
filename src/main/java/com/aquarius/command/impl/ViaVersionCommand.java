...
import com.aquarius.via.AquariusViaCommandSender;
...
public class ViaVersionCommand extends Command {
            ...
            .then(literal("zenithToServer")
                .then(argument("toggle", toggle()).executes(c -> {
                    ...
                    c.getSource().getEmbed()
                        .title("Aquarius To Server ViaVersion " + toggleStrCaps(CONFIG.client.viaversion.enabled));
                    return OK;
                }))
                .then(literal("disableOn2b2t")
                    .then(argument("toggle", toggle()).executes(c -> {
                        ...
                        c.getSource().getEmbed()
                            .title("Aquarius To Server Disable On 2b2t " + toggleStrCaps(CONFIG.client.viaversion.disableOn2b2t));
                        return OK;
                    })))
                .then(literal("autoConfig")
                    .then(argument("toggle", toggle()).executes(c -> {
                        ...
                        c.getSource().getEmbed()
                            .title("Aquarius To Server ViaVersion AutoConfig " + toggleStrCaps(CONFIG.client.viaversion.autoProtocolVersion));
                        return OK;
                    })))
                .then(literal("version")
                    .then(argument("version", wordWithChars()).executes(c -> {
                        ...
                        c.getSource().getEmbed()
                            .title("Aquarius To Server ViaVersion Version Updated!");
                        return OK;
                    }))))
            .then(literal("playerToAquarius")
                .then(argument("toggle", toggle()).executes(c -> {
                    CONFIG.server.viaversion.enabled = getToggle(c, "toggle");
                    c.getSource().getEmbed()
                        .title("Player To Aquarius ViaVersion " + toggleStrCaps(CONFIG.server.viaversion.enabled));
                    return OK;
                })))
            .then(literal("c").then(argument("args", greedyString()).executes(c -> {
                var sender = new AquariusViaCommandSender(c.getSource());
    ...
    public void defaultEmbed(final Embed embedBuilder) {
        embedBuilder
            .addField("Aquarius To Server ViaVersion", toggleStr(CONFIG.client.viaversion.enabled), false)
            .addField("Aquarius To Server Disable On 2b2t", toggleStr(CONFIG.client.viaversion.disableOn2b2t), false)
            .addField("Aquarius To Server Version", CONFIG.client.viaversion.autoProtocolVersion
            ...
            .addField("Player To Aquarius ViaVersion", toggleStr(CONFIG.server.viaversion.enabled), false)
            .primaryColor();
    }}
