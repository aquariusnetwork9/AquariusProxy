...
import com.aquarius.event.chat.DeathMessageChatEvent;
import com.aquarius.event.client.*;
import com.aquarius.event.module.*;
import com.aquarius.event.player.*;
import com.aquarius.event.plugin.PluginLoadFailureEvent;
import com.aquarius.event.plugin.PluginLoadedEvent;
...
import static com.aquarius.command.impl.StatusCommand.getCoordinates;
import static com.aquarius.discord.DiscordBot.*;
import static com.aquarius.util.math.MathHelper.formatDuration;
...
public class NotificationEventListener {
            ...
            switch (category) {
                case KICK -> {
                    if (!Proxy.getInstance().isPrio()) {
                      ...
                      Or migrate AquariusProxy instances to multiple hosts/IP's.
                      """);
                      ...
                      """);
                        ...
                        """.formatted(MODULE.get(SessionTimeLimit.class).getSessionTimeLimit().toHours()));
                            ...
                            if (!MODULE.get(AntiAFK.class).isEnabled()) {
                                msg += "\n\nConsider enabling AquariusProxy's AntiAFK module: `antiAFK on`";
                            }
    ...
    public void handleProxyClientConnectedEventCheck2b2tMCVersionMatch(PlayerConnectedEvent event) {
        ...
        if (!clientProtocolVersion.equalTo(playerProtocolVersion)) {
            var desc = """
                 **Client MC Version**: %s
                 **AquariusProxy Client MC Version**: %s

                 It is recommended to use the same MC version as the AquariusProxy client.

                 Otherwise you may experience issues with 2b2t's anti-cheat, which changes its checks based on client MC version.

                 Or configure AquariusProxy's client ViaVersion (reconnect after changing):
    ...
    public void handleProxyLoginFailedEvent(ClientLoginFailedEvent event) {
        ...
        If that fails, log into the account with the vanilla MC launcher and join a server. Then try again with AquariusProxy.
