...
import com.aquarius.plugin.api.PluginInstance;
...
public class PluginManager {
    ...
    public @Nullable AquariusProxyPlugin getPlugin(String id) {
        var instance = pluginInstances.get(id);
        if (instance == null) return null;
        return instance.getPluginInstance();
    }

    public String getId(final AquariusProxyPlugin pluginInstance) {
    ...
    public @Nullable PluginInfo getPluginInfo(final AquariusProxyPlugin pluginInstance) {
    ...
    protected void loadPlugin(final PluginInstance pluginInstance) {
        try {
            ...
            if (!AquariusProxyPlugin.class.isAssignableFrom(pluginClass)) {
                throw new RuntimeException("Plugin does not implement AquariusProxyPlugin interface");
            }
            AquariusProxyPlugin plugin;

            if (isKotlinObject(pluginClass)) {
                plugin = (AquariusProxyPlugin) getKotlinObject(pluginClass);
            } else {
                plugin = (AquariusProxyPlugin) pluginClass.getDeclaredConstructor().newInstance();
            }
