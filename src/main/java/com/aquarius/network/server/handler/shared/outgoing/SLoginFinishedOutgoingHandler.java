...
import com.aquarius.event.player.BlacklistedPlayerConnectedEvent;
import com.aquarius.event.player.NonWhitelistedPlayerConnectedEvent;
    ...
    public ClientboundLoginFinishedPacket apply(@NonNull ClientboundLoginFinishedPacket packet, @NonNull ServerSession session) {
        try {
            ...
            if (session.isTransferring()) {
                var transferSrc = session.getCookieCache().getAquariusTransferSrc();
                transferSrc.ifPresent(s -> SERVER_LOG.info("{} transferring from AquariusProxy instance: {}", clientGameProfile.getName(), s));
                if (CONFIG.server.onlyAquariusTransfers && transferSrc.isEmpty()) {
                    // clients can spoof these cookies easily, but the whitelist would stop them anyway
                    SERVER_LOG.info("Blocking transfer from non-AquariusProxy source. Username: {} UUID: {} MC: {} [{}]", clientGameProfile.getName(), clientGameProfile.getIdAsString(), session.getMCVersion(), session.getRemoteAddress());
