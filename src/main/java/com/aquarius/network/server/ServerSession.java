package com.aquarius.network.server;
...
import com.aquarius.cache.data.cookie.CookieCache;
...
import com.aquarius.event.player.PlayerDisconnectedEvent;
import com.aquarius.event.player.SpectatorDisconnectedEvent;
import com.aquarius.feature.ratelimiter.LoginRateLimiter;
import com.aquarius.feature.ratelimiter.PacketRateLimiter;
import com.aquarius.feature.spectator.SpectatorEntityRegistry;
import com.aquarius.feature.spectator.entity.SpectatorEntity;
import com.aquarius.network.codec.PacketCodecRegistries;
...
public class ServerSession extends TcpServerSession {
    ...
    private static final String teamName = "AquariusProxy";
    private static final Component displayName = Component.text("AquariusProxy");
        ...
        try {
            if (CONFIG.server.packetRateLimiter.enabled) {
                if (getPacketRateLimiter().countPacket()) {
                    ...
                    disconnect(Component.text("[AquariusProxy] Packet rate limit exceeded"));
                    return;
                }
            }
            Packet p = packet;
    ...
    public void sendAsyncAlert(final String minimessage) {
        this.sendAsync(new ClientboundSystemChatPacket(
            ComponentSerializer.minimessage("<gray>[<aqua>AquariusProxy<gray>] <reset>" + minimessage), false));
    }
