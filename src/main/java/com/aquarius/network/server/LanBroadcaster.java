...
public class LanBroadcaster {
    ...
    public LanBroadcaster() {
        ...
        this.motdSupplier = Suppliers.memoizeWithExpiration(() -> {
            try {
                var serverStatusInfo = AquariusServerInfoBuilder.INSTANCE.buildInfo(null);
            ...
            var bytes = ("[MOTD] AquariusProxy - " + CONFIG.authentication.username + "[/MOTD][AD]" + CONFIG.server.bind.port + "[/AD]").getBytes();
            return new DatagramPacket(bytes, bytes.length, broadcastAddressSupplier.get(), 4445);
        }, 10, TimeUnit.SECONDS);
    }
