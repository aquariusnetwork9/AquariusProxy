...
| **+ Members channel** | An optional **read-only** Discord channel that mirrors a **coordinate-scrubbed** subset of notifications (online/offline/queue/prio/pearl-pulled/visual-range) for non-admin members. RBAC-gated by an **audience role** — coords + proxy IP stripped below their threshold (visual-range coords have a dedicated off-switch); commands never accepted there. Ships **off**. See the [Discord Bot guide](https://github.com/aquariusnetwork9/AquariusProxy/wiki/Discord-Bot-Guide#members-channel-read-only-feed). |
